...
export default async function NewSponsorPage({
  ...
  if (!exhibitionId) {
    ...
    return (
      <div className="p-4 md:p-8 pt-20 md:pt-8">
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">新增贊助商</h1>
          <p className="text-gray-600 mt-2">請選擇要添加贊助商的展覽</p>
        </div>

        {exhibitions.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            <p className="text-gray-500 mb-4">您目前沒有可以添加贊助商的展覽</p>
            <a
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {exhibitions.map((exhibition) => (
              <a
                key={exhibition.id}
                href={`/admin/sponsors/new?exhibitionId=${exhibition.id}`}
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">
                    {exhibition._count.sponsors} 個贊助商
                  </span>
                </div>
              </a>
            ))}
  ...
  return (
    <div className="p-4 md:p-8 pt-20 md:pt-8">
      ...
      <SponsorForm mode="create" exhibitionId={exhibitionId} />
    </div>
  )
}
