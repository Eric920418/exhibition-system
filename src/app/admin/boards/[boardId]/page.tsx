  ...
  const completedCards = board.lists.reduce(
    (sum, list) =>
      ...
      list.cards.filter((card) => {
        if (card.subtasks.length === 0) return false
        return card.subtasks.every((s) => s.completed)
      }).length,
    0
  )
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-6 text-sm text-gray-600">
              <div className="flex items-center gap-2">
                ...
                <span>
                  <strong>{board.lists.length}</strong> 列表
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span>
                  <strong>{totalCards}</strong> 卡片
                </span>
                {totalCards > 0 && (
            ...
            <div className="flex rounded-lg border overflow-hidden">
              <button className="px-3 py-1.5 text-sm bg-blue-50 text-blue-600 border-r">
                <LayoutGrid className="w-4 h-4" />
              </button>
              <Link
                href={`/admin/boards/${boardId}/calendar`}
