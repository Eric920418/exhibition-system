...
/**
 * POST /api/reservations/serving/[teamId]/complete
 * 標記預約完成（需認證）
 */
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!['CALLED', 'IN_PROGRESS'].includes(reservation.status)) {
      return apiError(
        `無法標記完成：預約狀態為「${reservation.status}」`,
        400
      )
    }
    ...
    await createAuditLog({
      action: AuditAction.RESERVATION_COMPLETE,
      userId: user.id,
